...
describe("hills endpoint classification - country", () => {
    ...
    classList.forEach((input) => {
        it("GET /hills/" +
            ...
            const response = yield request(app).get("/hills/" + input[0]);
            expect(response.status).toBe(200);
            expect(response.body.hills.every((currentValue) => currentValue.Country == input[1])).toBe(true);
        }));
    });
});
...
describe("hills endpoint classification - default", () => {
    it("GET /hills/RandomString should return a list of all hills", () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        expect(response.status).toBe(200);
        let list = [];
        response.body.hills.forEach((object) => {
            list.push(object.Number);
        });
        expect(isAscending(list)).toBe(true);
    }));
    it("GET /hills/all should return a list of all hills", () => __awaiter(void 0, void 0, void 0, function* () {
        const response = yield request(app).get("/hills/all");
        expect(response.status).toBe(200);
        let list = [];
        response.body.hills.forEach((object) => {
...
describe("hills endpoint direction", () => {
    ...
    directionList.forEach((input) => {
        it("GET /hills/all?d=" +
            ...
            input[2] +
            " by " +
            input[0], () => __awaiter(void 0, void 0, void 0, function* () {
            const response = yield request(app).get("/hills/all?d=" + input[1]);
            expect(response.status).toBe(200);
...
describe("hills endpoint classification - default", () => {
    it("GET /hills/all?d='RandomString' should return a list of all hills ordered by number", () => __awaiter(void 0, void 0, void 0, function* () {
        const response = yield request(app).get("/hills/all?d=randomString");
        expect(response.status).toBe(200);
        let list = [];
        response.body.hills.forEach((object) => {
            list.push(object.Number);
...
function checkDirectionInt(array, input) {
    let list = [];
...
function isAlphabetic(arr) {
    let sortedList = arr.sort();
    return sortedList == arr;
}
function isReverseAlphabetic(arr) {
    let sortedList = arr.sort();
